...
d3.json("custom.geo.json").then(function (worldMap) {
    ...
    d3.csv("choropleth_map.csv").then(function (data) {
        ...
        // Extract unique countries for the country search dropdown
        var uniqueCountries = Array.from(new Set(data.map(function (d) {
            return d.Country;
        }))).sort();

        // Populate country search dropdown
        var countryDropdown = d3.select("#country-search");
        ...
        countryDropdown.on("change", function () {
            ...
            if (selectedCountry) {
                var selectedCountryData = worldMap.features.find(function (d) {
                    return d.properties.admin === selectedCountry;
                });
                ...
                if (selectedCountryData) {
                    ...
                    var bounds = path.bounds(selectedCountryData);
                ...
                svg.selectAll("path[data-name='" + selectedCountry + "']")
    ...
    var paths = svg.selectAll("path")
        ...
        .enter().append("path")
        .attr("d", path)
        .attr("data-name", function(d) { return d.properties.admin; }) // Add data attribute for country name
        .style("fill", function (d) {
            var countryData = data.find(function (item) {
